...
<div class="container">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"> Mise à jour d'un produit </h3>
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form class="needs-validation" method="post" action="{{ route('produits.update', $produit->id) }}" accept-charset='UTF-8'>
                        @csrf
                        @method('PUT')
                             ...
                             <div class="form-group row d-flex align-items-center mb-5">
                                ...
                                <div class="col-lg-5">
                                    <input type="text" name='nom' class="form-control" value="{{$produit->nomProduit }}"  required="required">
                                    @if ($errors->has('nom'))
                            ...
                            <div class="form-group row d-flex align-items-center mb-5">
                                ...
                                <div class="col-lg-5">
                                    <input type="number"  name='prix' class="form-control" value="{{ $produit->prixProduit }}"  required="required">
                                    @if ($errors->has('prix'))
                                    <span class="help-block">
                                    ...
                                    <select name='user_id' id="user" class="form-control custom-select" required>
                                        ...
                                        @if(!empty($users))
                                            @foreach($users as $user)
                                             <option value="{{$user->id}}" {{ $produit->user_id == $user->id ? 'selected' : '' }} >{{$user->name }} </option>
